...
public class JarFile extends AbstractJarFile implements Iterable<java.util.jar.JarEntry> {
   ...
   private JarFile(
      ...
      this.manifestSupplier = manifestSupplier != null ? manifestSupplier : () -> {
         try {
            Manifest var2x;
            try (InputStream inputStream = this.getInputStream("META-INF/MANIFEST.MF")) {
               if (inputStream == null) {
                  return null;
               }

               var2x = new Manifest(inputStream);
            }

            return var2x;
         } catch (IOException var6xx) {
            throw new RuntimeException(var6xx);
         }
      };
   }
   ...
   public synchronized InputStream getInputStream(ZipEntry entry) throws IOException {
      this.ensureOpen();
      return entry instanceof JarEntry jarEntry ? this.entries.getInputStream(jarEntry) : this.getInputStream(entry != null ? entry.getName() : null);
   }
